...
### Beware this will take a really long tims
# tagme()
...
tbl.458.TRBL <- tbl(proj.458.sql.motus, "alltags") %>%
  filter(speciesEN == "Tricolored Blackbird") %>%
  collect()

# Get the daily median location of GPS points for these data
TRBL.GPS <- getGPS(src = proj.458.sql.motus, data = tbl.458.TRBL)
...
df.alltags.458.01_22 = df.458.alltags
# export as RDS to preserve dateTimes
# You can then just import this to work with tag data from before the date of export
saveRDS(df.alltags.458.01_22, "df_alltags_458.01_22.rds")
# To read it back in
# df.alltags.458.01_22 <- readRDS("df_alltags_458.01_22.rds")
```
