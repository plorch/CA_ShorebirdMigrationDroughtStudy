...
cosumnes.motus = tagme("CTT-D00F694B4918", update = T)
colusa.motus = tagme("CTT-9697596E76D6", update = T)
birdhaven.motus = tagme("CTT-F51A7C862D57", update = T)
...
p.sig
pixley.sig  = df.pixley.alltags.56719 %>% 
  ggplot(aes(x = ts, y = sig, col = as.factor(antBearing))) +
  theme_bw() + 
  geom_point() + 
  labs(x = "Time of day", y = "Signal strength") +
  ...
  facet_grid(recvDeployName ~ .)
pixley.sig
pixley.sig  = df.pixley.alltags.57234 %>% 
  ggplot(aes(x = ts, y = sig, col = as.factor(antBearing))) +
  theme_bw() + 
  geom_point() + 
  labs(x = "Time of day", y = "Signal strength") +
