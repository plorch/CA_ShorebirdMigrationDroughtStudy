...
napa.sonoma.motus = tagme("CTT-21FFFAC8B255", new=TRUE, update = T)
pixley.motus = tagme("CTT-140682CCEB52", new=TRUE, update = T)
kern.motus = tagme("CTT-8F11AA6622E1", new=TRUE, update = T)
...
# To work with existing sqlite databases
napa.sonoma.motus = tagme("CTT-21FFFAC8B255", new=FALSE, update = FALSE)
pixley.motus = tagme("CTT-140682CCEB52", new=FALSE, update = FALSE)
kern.motus = tagme("CTT-8F11AA6622E1", new=FALSE, update = FALSE)
cosumnes.motus = tagme("CTT-D00F694B4918", new=FALSE, update = FALSE)
colusa.motus = tagme("CTT-9697596E76D6",  new=FALSE, update = FALSE)
birdhaven.motus = tagme("CTT-F51A7C862D57",  new=FALSE, update = FALSE)
...
df.pixley.alltags <- tbl.pixley.alltags %>% 
  ...
  as.data.frame() %>%     # for all fields in the df (data frame)
  mutate(ts = as_datetime(ts, tz = "UTC", origin = "1970-01-01"))
df.kern.alltags <- tbl.kern.alltags %>% 
  collect() %>% 
  as.data.frame() %>%     # for all fields in the df (data frame)
  mutate(ts = as_datetime(ts, tz = "UTC", origin = "1970-01-01"))
df.cosumnes.alltags <- tbl.cosumnes.alltags %>% 
  collect() %>% 
  as.data.frame() %>%     # for all fields in the df (data frame)
  mutate(ts = as_datetime(ts, tz = "UTC", origin = "1970-01-01"))
df.colusa.alltags <- tbl.colusa.alltags %>% 
  collect() %>% 
  as.data.frame() %>%     # for all fields in the df (data frame)
  mutate(ts = as_datetime(ts, tz = "UTC", origin = "1970-01-01"))
df.birdhaven.alltags <- tbl.birdhaven.alltags %>% 
  collect() %>% 
  as.data.frame() %>%     # for all fields in the df (data frame)
  mutate(ts = as_datetime(ts, tz = "UTC", origin = "1970-01-01"))
...
# I think Rodd did not track at Pixley, but we were together
...
# I got these from Motus website https://motus.org/data/project/tags
mfg_tag_id_tnc = c("2D342D78", "612A072A") # Rodd's test tags
motus_tag_id_tnc = c(57234, 58286)
tag_model_tnc = c("LifeTag", "HybridTag")
...
# Pat tested, Rodd?
df.kern.alltags.56719 = df.kern.alltags %>%
  filter(motusTagID == 56719,
         ts > "2022-07-12",
         ts < "2022-07-15")

# Rodd only, LifeTag
df.cosumnes.alltags.57234 = df.cosumnes.alltags %>%
  filter(motusTagID == 57234,
         ts > "2022-07-26",
         ts < "2022-07-28")
# Rodd only, HybridTag
df.cosumnes.alltags.58286 = df.cosumnes.alltags %>%
  filter(motusTagID == 58286,
         ts > "2022-07-26",
         ts < "2022-07-28")

# Rodd only, HybridTag
df.colusa.alltags.58286 = df.colusa.alltags %>%
  filter(motusTagID == 58286,
         ts > "2022-08-17",
         ts < "2022-08-19")

# Rodd only, HybridTag
df.birdhaven.alltags.58286 = df.birdhaven.alltags %>%
  filter(motusTagID == 58286,
         ts > "2022-08-17",
         ts < "2022-08-19")
...
pixley.57234.sig
## Kern
kern.56719.sig  = df.kern.alltags.56719 %>% 
  ggplot(aes(x = ts, y = sig, col = as.factor(antBearing))) +
  theme_bw() + 
  geom_point() + 
  labs(x = "Time of day", y = "Signal strength") +
...
kern.56719.sig
## Cosumnes 
cosumnes.57234.sig  = df.cosumnes.alltags.57234 %>% 
  ggplot(aes(x = ts, y = sig, col = as.factor(antBearing))) +
  theme_bw() + 
  geom_point() + 
  labs(x = "Time of day", y = "Signal strength") +
  ...
  facet_grid(recvDeployName ~ .)
cosumnes.57234.sig
cosumnes.58286.sig  = df.cosumnes.alltags.58286 %>% 
  ggplot(aes(x = ts, y = sig, col = as.factor(antBearing))) +
  theme_bw() + 
  geom_point() + 
  labs(x = "Time of day", y = "Signal strength") +
...
cosumnes.58286.sig
## Colusa
colusa.58286.sig  = df.colusa.alltags.58286 %>% 
  ggplot(aes(x = ts, y = sig, col = as.factor(antBearing))) +
  theme_bw() + 
  geom_point() + 
  labs(x = "Time of day", y = "Signal strength") +
...
colusa.58286.sig
## Birdhaven
birdhaven.58286.sig  = df.birdhaven.alltags.58286 %>% 
  ggplot(aes(x = ts, y = sig, col = as.factor(antBearing))) +
  theme_bw() + 
  geom_point() + 
  labs(x = "Time of day", y = "Signal strength") +
...
points.kernNW.gpx@data$tmp = round_date(points.kernNW.gpx@data$timestamp, "minute")
df.kern.alltags.56719$tmp = round_date(df.kern.alltags.56719$ts, "minute")
points.kernNWR.gpx@data$tmp = round_date(points.kernNWR.gpx@data$timestamp, "minute")
points.cosumnes.gpx@data$tmp = round_date(points.cosumnes.gpx@data$timestamp, "minute")
df.cosumnes.alltags.57234$tmp = round_date(df.cosumnes.alltags.57234$ts, "minute")
df.cosumnes.alltags.58286$tmp = round_date(df.cosumnes.alltags.58286$ts, "minute")
points.colusa.gpx@data$tmp = round_date(points.colusa.gpx@data$timestamp, "minute")
df.colusa.alltags.58286$tmp = round_date(df.colusa.alltags.58286$ts, "minute")
points.birdhaven.gpx@data$tmp = round_date(points.birdhaven.gpx@data$timestamp, "minute")
df.birdhaven.alltags.58286$tmp = round_date(df.birdhaven.alltags.58286$ts, "minute")

## Add group
points@data$grp = 1:nrow(points@data)
points.pixley.gpx@data$grp = 1:nrow(points.pixley.gpx@data)
points.kernNW.gpx@data$grp = 1:nrow(points.kernNW.gpx@data)
points.kernNWR.gpx@data$grp = 1:nrow(points.kernNWR.gpx@data)
points.cosumnes.gpx@data$grp = 1:nrow(points.cosumnes.gpx@data)
points.colusa.gpx@data$grp = 1:nrow(points.colusa.gpx@data)
points.birdhaven.gpx@data$grp = 1:nrow(points.birdhaven.gpx@data)
...
points.pixley.56719.merged = merge(points.pixley.gpx@data, df.pixley.alltags.56719, "tmp") 
points.pixley.56719.merged.nearest = points.pixley.56719.merged %>%
  group_by(grp) %>% 
  slice(which.min(abs(difftime(timestamp, ts)))) %>% 
  ungroup() %>% 
...
points.pixley.56719 = merge(points.pixley.gpx, points.pixley.56719.merged.nearest, "grp")
plot(points.pixley.56719)

points.pixley.57234.merged = merge(points.pixley.gpx@data, df.pixley.alltags.57234, "tmp") 
points.pixley.57234.merged.nearest = points.pixley.57234.merged %>%
  group_by(grp) %>% 
  slice(which.min(abs(difftime(timestamp, ts)))) %>% 
  ungroup() %>% 
  select(-c(tmp, track_fid, track_seg_id, time, "magvar", "geoidheight", "name", 
            ...
            tagDeployStart, tagDeployEnd, tagDepLat, tagDepLon, tagDepAlt, 
            tagDepComments, tagDeployTest, fullID))
points.pixley.57234 = merge(points.pixley.gpx, points.pixley.57234.merged.nearest, "grp")
plot(points.pixley.57234)


points.kernNW.merged = merge(points.kernNW.gpx@data, df.kern.alltags.56719, "tmp") 
points.kernNW.merged.nearest = points.kernNW.merged %>%
  group_by(grp) %>% 
  slice(which.min(abs(difftime(timestamp, ts)))) %>% 
  ungroup() %>% 
  select(-c(tmp, track_fid, track_seg_id, time, "magvar", "geoidheight", "name", 
            ...
            tagDeployStart, tagDeployEnd, tagDepLat, tagDepLon, tagDepAlt, 
            tagDepComments, tagDeployTest, fullID))
points.kernNW = merge(points.kernNW.gpx, points.kernNW.merged.nearest, "grp")
plot(points.kernNW)

points.kernNWR.merged = merge(points.kernNWR.gpx@data, df.kern.alltags.56719, "tmp") 
points.kernNWR.merged.nearest = points.kernNWR.merged %>%
  group_by(grp) %>% 
  slice(which.min(abs(difftime(timestamp, ts)))) %>% 
  ungroup() %>% 
  select(-c(tmp, track_fid, track_seg_id, time, "magvar", "geoidheight", "name", 
            ...
            tagDeployStart, tagDeployEnd, tagDepLat, tagDepLon, tagDepAlt, 
            tagDepComments, tagDeployTest, fullID))
points.kernNWR = merge(points.kernNWR.gpx, points.kernNWR.merged.nearest, "grp")
plot(points.kernNWR)

points.cosumnes.57234.merged = merge(points.cosumnes.gpx@data, df.cosumnes.alltags.57234, "tmp") 
points.cosumnes.57234.merged.nearest = points.cosumnes.57234.merged %>%
  group_by(grp) %>% 
  slice(which.min(abs(difftime(timestamp, ts)))) %>% 
  ungroup() %>% 
  select(-c(tmp, track_fid, track_seg_id, time, "magvar", "geoidheight", "name", 
            ...
            tagDeployStart, tagDeployEnd, tagDepLat, tagDepLon, tagDepAlt, 
            tagDepComments, tagDeployTest, fullID))
points.cosumnes.57234 = merge(points.cosumnes.gpx, points.cosumnes.57234.merged.nearest, "grp")
plot(points.cosumnes.57234)

points.cosumnes.58286.merged = merge(points.cosumnes.gpx@data, df.cosumnes.alltags.58286, "tmp") 
points.cosumnes.58286.merged.nearest = points.cosumnes.58286.merged %>%
  group_by(grp) %>% 
  slice(which.min(abs(difftime(timestamp, ts)))) %>% 
  ungroup() %>% 
  select(-c(tmp, track_fid, track_seg_id, time, "magvar", "geoidheight", "name", 
            ...
            tagDeployStart, tagDeployEnd, tagDepLat, tagDepLon, tagDepAlt, 
            tagDepComments, tagDeployTest, fullID))
points.cosumnes.58286 = merge(points.cosumnes.gpx, points.cosumnes.58286.merged.nearest, "grp")
plot(points.cosumnes.58286)

points.colusa.58286.merged = merge(points.colusa.gpx@data, df.colusa.alltags.58286, "tmp") 
points.colusa.58286.merged.nearest = points.colusa.58286.merged %>%
  group_by(grp) %>% 
  slice(which.min(abs(difftime(timestamp, ts)))) %>% 
  ungroup() %>% 
  select(-c(tmp, track_fid, track_seg_id, time, "magvar", "geoidheight", "name", 
            ...
            tagDeployStart, tagDeployEnd, tagDepLat, tagDepLon, tagDepAlt, 
            tagDepComments, tagDeployTest, fullID))
points.colusa.58286 = merge(points.colusa.gpx, points.colusa.58286.merged.nearest, "grp")
plot(points.colusa.58286)

points.birdhaven.58286.merged = merge(points.birdhaven.gpx@data, df.birdhaven.alltags.58286, "tmp") 
points.birdhaven.58286.merged.nearest = points.birdhaven.58286.merged %>%
  group_by(grp) %>% 
  slice(which.min(abs(difftime(timestamp, ts)))) %>% 
  ungroup() %>% 
  select(-c(tmp, track_fid, track_seg_id, time, "magvar", "geoidheight", "name", 
            ...
            tagDeployStart, tagDeployEnd, tagDepLat, tagDepLon, tagDepAlt, 
            tagDepComments, tagDeployTest, fullID))
points.birdhaven.58286 = merge(points.birdhaven.gpx, points.birdhaven.58286.merged.nearest, "grp")
plot(points.birdhaven.58286)
...
pal1 <- colorNumeric(
  palette = "Reds",
  domain = points.pixley.57234$sig)
pal2 <- colorNumeric(
  palette = "Blues",
  domain = points.pixley.56719$sig)

leaflet(points.pixley.57234) %>%
  addTiles(group = "OSM (default)") %>%
  addProviderTiles('Esri.WorldImagery',group = "Esri World Imagery") %>%
  # addMarkers()
  # This only produced one color
  # addAwesomeMarkers(icon=icons, label=~as.character(sig))
  addCircleMarkers(color = ifelse(is.na(points.pixley.57234$sig),"#737373",pal1(points.pixley.57234$sig)),
                   radius = ifelse(is.na(points.pixley.57234$sig), 2, 10),
                   popup = ~as.character(points.pixley.57234$sig), 
                   label = ~as.character(points.pixley.57234$sig),
                   group = "LifeTag 57234 on window") %>%
  addCircleMarkers(color = ifelse(is.na(points.pixley.56719$sig),"#737373",pal2(points.pixley.56719$sig)),
                   radius = ifelse(is.na(points.pixley.56719$sig), 2, 10),
                   popup = ~as.character(points.pixley.56719$sig), 
                   label = ~as.character(points.pixley.56719$sig),
                   group = "LifeTag 56719 on aerial") %>%
  addLayersControl(
        baseGroups = c("OSM (default)", "Esri World Imagery"),
        overlayGroups = c("LifeTag 57234 on window", "LifeTag 56719 on aerial"),
...
pal <- colorNumeric(
  palette = "Reds",
  domain = points.kernNWR$sig)

leaflet(points.kernNWR) %>% # points.kernNW
  addTiles(group = "OSM (default)") %>%
  addProviderTiles('Esri.WorldImagery',group = "Esri World Imagery") %>%
  addCircleMarkers(color = ifelse(is.na(points.kernNWR$sig),"#737373",pal(points.kernNWR$sig)),
                   radius = ifelse(is.na(points.kernNWR$sig), 2, 10),
                   popup = ~as.character(points.kernNWR$sig), 
                   label = ~as.character(points.kernNWR$sig),
                   group = "Kern NWR") %>%
  # There appear to be no signals for this visit.
  # addCircleMarkers(color = ifelse(is.na(points.kernNW$sig),"#737373",pal(points.kernNW$sig)),
  #                  radius = ifelse(is.na(points.kernNW$sig), 2, 10),
  #                  popup = ~as.character(points.kernNW$sig), 
  #                  label = ~as.character(points.kernNW$sig),
  #                  group = "Kern NW") %>%
  addLayersControl(
        baseGroups = c("OSM (default)", "Esri World Imagery"),
        # overlayGroups = c("Kern NWR", "Kern NW"),
        position = "bottomright",
        options = layersControlOptions(collapsed = TRUE))
...
pal1 <- colorNumeric(
  palette = "Reds",
  domain = points.cosumnes.57234$sig)
pal2 <- colorNumeric(
  palette = "Blues",
  domain = points.cosumnes.58286$sig)

leaflet(points.cosumnes.57234) %>%
  addTiles(group = "OSM (default)") %>%
  addProviderTiles('Esri.WorldImagery',group = "Esri World Imagery") %>%
  # addMarkers()
  # This only produced one color
  # addAwesomeMarkers(icon=icons, label=~as.character(sig))
  addCircleMarkers(color = ifelse(is.na(points.cosumnes.57234$sig),"#737373",pal1(points.cosumnes.57234$sig)),
                   radius = ifelse(is.na(points.cosumnes.57234$sig), 2, 10),
                   popup = ~as.character(points.cosumnes.57234$sig), 
                   label = ~as.character(points.cosumnes.57234$sig),
                   group = "LifeTag 57234 on 2m PVC") %>%
  addCircleMarkers(color = ifelse(is.na(points.cosumnes.58286$sig),"#737373",pal2(points.cosumnes.58286$sig)),
                   radius = ifelse(is.na(points.cosumnes.58286$sig), 2, 10),
                   popup = ~as.character(points.cosumnes.58286$sig), 
                   label = ~as.character(points.cosumnes.58286$sig),
                   group = "HybridTag 58286 on 2m PVC") %>%
  addLayersControl(
        baseGroups = c("OSM (default)", "Esri World Imagery"),
        overlayGroups = c("LifeTag 57234 on 2m PVC", "HybridTag 58286 on 2m PVC"),
        position = "bottomright",
        options = layersControlOptions(collapsed = TRUE))
...
pal <- colorNumeric(
  palette = "Reds",
  domain = points.colusa.58286$sig)

leaflet(points.colusa.58286) %>%
  addTiles(group = "OSM (default)") %>%
  addProviderTiles('Esri.WorldImagery',group = "Esri World Imagery") %>%
  addCircleMarkers(color = ifelse(is.na(points.colusa.58286$sig),"#737373",pal(points.colusa.58286$sig)),
                   radius = ifelse(is.na(points.colusa.58286$sig), 2, 10),
                   popup = ~as.character(points.colusa.58286$sig), 
                   label = ~as.character(points.colusa.58286$sig)) %>%
  addLayersControl(
        baseGroups = c("OSM (default)", "Esri World Imagery"),
        position = "bottomright",
        options = layersControlOptions(collapsed = TRUE))
...
pal <- colorNumeric(
  palette = "Reds",
  domain = points.birdhaven.58286$sig)

leaflet(points.birdhaven.58286) %>%
  addTiles(group = "OSM (default)") %>%
  addProviderTiles('Esri.WorldImagery',group = "Esri World Imagery") %>%
  addCircleMarkers(color = ifelse(is.na(points.birdhaven.58286$sig),"#737373",pal(points.birdhaven.58286$sig)),
                   radius = ifelse(is.na(points.birdhaven.58286$sig), 2, 10),
                   popup = ~as.character(points.birdhaven.58286$sig), 
                   label = ~as.character(points.birdhaven.58286$sig)) %>%
  addLayersControl(
        baseGroups = c("OSM (default)", "Esri World Imagery"),
        position = "bottomright",
        options = layersControlOptions(collapsed = TRUE))
