...
# To just connect and use the current .motus SQLite fiels use this
...
tags_points.nearest = tags_points_l_joined %>%
  ...
  select("track_seg_point_id", "ele", "timestamp", "grp", "hitID", "runID",
         "batchID", "sig", "done", "motusTagID", "ambigID", "port", "nodeNum",
         "runLen", "motusFilter", "bootnum", "deviceID", "recvDeployID",
         "recvDeployLat", "recvDeployLon", "recvDeployAlt", "recv", 
         "recvDeployName" , "recvSiteName", "isRecvMobile", "recvProjID",
...
m = leaflet(points) %>%
  ...
  addCircleMarkers(color = pal(points$sig),
                   radius = ifelse(is.na(points$sig), 2, 
                                   -(points$sig - max(na.omit(points$sig)))),
